	short channelType = 1;
	short channelIdx = 0;
	char channelIP[16] = "127.0.0.1";
	long channelPort = 20000;
	char channelName[29] = "Test Channel";
	char channelGuild[13] = "HKGolden";
	...
	memcpy(myData+0x18-0x14, &channelIP, strlen(channelIP)+1);
	memcpy(myData+0x28-0x14, &channelPort, 4);
	memcpy(myData+0x2C-0x14, &channelPopulation, 4);
	memcpy(myData+0x30-0x14, &channelName, strlen(channelName)+1);
	memcpy(myData+0x4D-0x14, &channelGuild, strlen(channelGuild)+1);
