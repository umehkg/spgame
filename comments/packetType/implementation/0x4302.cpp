if( *(long*)&decryptBuff[4] == 0x4301){
	//declarations
	unsigned char myData1[0x979];
	char *myData = (char *)calloc(0x979-0x14, 0x1);
	//const
	...
	long playerAvatarEquipIdx[7] = {-1, -1, -1, -1, -1, -1, -1}; //Idx not ItemId!!!
	unsigned char playerEventFlags[7] = {0, 0, 0, 0, 0, 0, 0}; //see debug mode
	...
	memcpy(myData+0x928-0x14, new long(1), 4); //>0
	memcpy(myData+0x92C-0x14, new long(4), 4); //switch 1~4
	memcpy(myData+0x930-0x14, new long(0), 4); //?
	memcpy(myData+0x934-0x14, new long(0), 4); //?
	memcpy(myData+0x938-0x14, new long(0), 4); //?
	
	memcpy(myData+0x93C-0x14, &playerEventFlags[0], 1*7);
	memcpy(myData+0x944-0x14, new long(0), 4); //+cb4 ?
	memcpy(myData+0x94C-0x14, &lobbyMaxRooms, 4);
	memcpy(myData+0x950-0x14, &playerAvatarEquipIdx[0], 4*7);
	
	memcpy(myData+0x96C-0x14, new long(0), 4); //?
	memcpy(myData+0x974-0x14, &playerType, 4);
	memcpy(myData+0x978-0x14, new unsigned char(0), 1); //ds:byte_821858 (some korean annnouncementWnd msg...)
	
	
	CPacket *myPacket = new CPacket(0x4302, myData, 0x979-0x14);
	free(myData);
	myPacket->Encrypt();
	myPacket->Output((char*)myData1, 0x979);
